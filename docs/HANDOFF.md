...
- `cep/` — panel: `index.html`, `js/main.js` (UI + cola `HPQueue` + estado), `js/store.js` (`HPStore`, persiste por proyecto+secuencia en localStorage), `js/transcript.js`, `jsx/host.jsx` (ExtendScript: leer marcadores, mover playhead, insertar/recolorear clips, exportar frame), `css/style.css`.
