...
- **Panel CEP** (`cep/`) — la interfaz dentro de Premiere (`com.codigo.hyperpremiere`), con
  Node embebido (`--enable-nodejs --mixed-context`). `js/main.js` (UI + cola `HPQueue` +
  estado), `js/store.js` (`HPStore`, persiste por proyecto+secuencia), `js/transcript.js`,
  `css/style.css`. Tooltips propios (CEF no dibuja los `title` nativos).
